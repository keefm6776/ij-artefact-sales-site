...
class TestCustomerForm(TestCase):
 # Test that form cannot be created with incomplete information
    ...
    def test_cannot_process_customer_addition_or_edit_with_just_a_full_name(self):
        form = CustomerForm({'full_name': 'pippy longstockings'})
        self.assertFalse(form.is_valid())

    def test_cannot_process_customer_addition_or_edit_with_just_street_Address1(self):
        form = CustomerForm({'street_Address1': '20 Anywhere Street'})
        self.assertFalse(form.is_valid())
    
    def test_cannot_process_customer_addition_or_edit_with_just_street_Address2(self):
        form = CustomerForm({'street_Address2': 'AnyDistrict'})
        self.assertFalse(form.is_valid())

    def test_cannot_process_customer_addition_or_edit_with_just_town_or_city(self):
        form = CustomerForm({'town_or_city': 'Limerick'})
        self.assertFalse(form.is_valid())
    
    def test_cannot_process_customer_addition_or_edit_with_just_a_county(self):
        form = CustomerForm({'county': 'Tipperary'})
        self.assertFalse(form.is_valid())

    def test_cannot_process_customer_addition_or_edit_with_just_a_country(self):
        form = CustomerForm({'country': 'Republic Of Ireland'})
        self.assertFalse(form.is_valid())

    def test_cannot_process_customer_addition_or_edit_with_just_a_postcode(self):
        form = CustomerForm({'postcode': 'D07EX90'})
        self.assertFalse(form.is_valid())
    
    def test_cannot_process_customer_addition_or_edit_with_just_a_phone_number(self):
        form = CustomerForm({'phone_number': '0871234567'})
        self.assertFalse(form.is_valid())
    
    def test_cannot_process_customer_addition_or_edit_with_just_an_email(self):
