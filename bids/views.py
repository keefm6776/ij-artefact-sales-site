...
def make_bid(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            (form.cleaned_data["bid"] > 0):
                ...
                """if bid is not greater than highest bid or zero/negative,"""
                """then prompt user of this"""
                if (form.cleaned_data["bid"] <= highest_bid):
                    form.add_error(None, "Your bid must be greater than the \
                    current highest bid!")

                if (form.cleaned_data["bid"] <= 0):
                    form.add_error(None, "Your bid cannot be zero or \
                    negative!")
        else:
            form = BidsForm()
            return redirect(for_sale_artefacts)
    ...
    return render(request, "make_bid.html", {'form': form,
                  'highest_bid': highest_bid, 'artefact': artefact_bid})
