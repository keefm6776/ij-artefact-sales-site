...
def sold_artefacts(request):
    """ Finds all sold artefacts (not despatched) in the database"""
...
def despatched_artefacts(request):
    """ Finds all sold artefacts that have been despatched """
    """ in the database and displays them """
    ...
    return render(request, "artefacts_despatched.html",
                  {"artefacts": artefacts})
...
def add_artefact(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            if ((form.cleaned_data["century"] > 0) and
               (form.cleaned_data["price"] > 0) and
               (form.cleaned_data["name"] != '')):
                ...
                if (form.cleaned_data["century"] <= 0):
                    form.add_error(None, "The Century Cannot be negative\
                                          or zero!")
                """ When Price Is Negative or Zero give apropriate error """
                if (form.cleaned_data["price"] <= 0):
                    form.add_error(None, "The Price Set Cannot be negative\
                                          or zero!")
                """ When Name is left blank give apropriate error,"""
                """ if not caught by HTML5 """
                if (form.cleaned_data["name"] == ''):
                    form.add_error(None, "The name field cannot be left\
                                           blank!")
                """ When description is left blank give apropriate error,"""
                """ if not caught by HTML5 """
                if (form.cleaned_data["description"] == ''):
                    form.add_error(None, "The description field cannot \
                                          be left blank!")
                """ When history is left blank give apropriate error, """
                """ if not caught by HTML5 """
                if (form.cleaned_data["history"] == ''):
                    form.add_error(None, "The history field cannot be \
                                          left blank!")
                    """ When era is not AD or BC give apropriate error"""
                if ((form.cleaned_data["era"] != 'AD') and
                   (form.cleaned_data["era"] != 'BC')):
...
def edit_artefact_detail(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if ((form.cleaned_data["century"] > 0) and
                (form.cleaned_data["price"] > 0) and
                (form.cleaned_data["name"] != '') and
                (form.cleaned_data["description"] != '') and
                (form.cleaned_data["history"] != '') and
               ((form.cleaned_data["era"] == 'AD') or
               (form.cleaned_data["era"] == 'BC'))):
                form.save()
                """If century and price are valid, ie not zero or -ve"""
                return redirect(for_sale_artefacts)
                ...
                if (form.cleaned_data["century"] <= 0):
                    form.add_error(None, "The Century Cannot be \
                                          negative or zero!")
                """ If century is negative or zero display """
                """ appropriate error """

                if (form.cleaned_data["price"] <= 0):
                    form.add_error(None, "The Price Set Cannot be \
                                          negative or zero!")
                """ If price is negative or zero display appropriate error """

                if (form.cleaned_data["name"] == ''):
                    form.add_error(None, "The name field cannot be\
                                           left blank!")
                """ When Name is left blank give apropriate error, """
                """ if not caught by HTML5 """
                ...
                if (form.cleaned_data["history"] == ''):
                    form.add_error(None, "The history field cannot\
                                           be left blank!")
                """ When History is left blank give apropriate error, """
                """ if not caught by HTML5 """
...
def despatch_artefact(request, id):
    ...
    """ Sending html to a pdf file adapted from code at """
    ...
    html_string = render_to_string(
        '../templates/despatch_template.html',
    ...
    with fs.open('despatch note.pdf') as pdf:
        ...
        response['Content-Disposition'] = \
            'attachment; filename="despatch note.pdf"'
        return response
...
def past_purchases(request):
    ...
    past_purchases = OrderLineItem.objects.\
        filter(order_id__in=list(user_orders)).values('artefact')
    """ Find the artefact ids that are contained in these orders """

    artefacts_sold_to_user = Artefact.objects.\
        filter(id__in=past_purchases).order_by('-id')
    ...
    return render(request, "past_purchases.html",
                  {"artefacts": artefacts_sold_to_user,
