...
def checkout(request, id):
    ...
    if request.method == "POST":
        ...
        if order_form.is_valid() and payment_form.is_valid():
            ...
            try:
                """try charging customer card/address details"""
                """with amount due"""
            ...
            if customer.paid:
                ...
                for id, quantity in cart.items():
                    """ for artefacts in cart, mark as sold when payment """
                    """ is successful """
                    artefact = get_object_or_404(Artefact, pk=id)
                    artefact.sold = True
                    artefact.save()
                ...
                """ if payment did not go through prompt user that it """
                """ was not successful """
        ...
        """ Initialising paymemnt form to User Customer Details adapted """
        """ from code at StackOverFlow.com """
        payment_form = MakePaymentForm()
        order_form = OrderForm()
        data = {'delivery_full_name': customer_object.full_name,
                'delivery_street_address1': customer_object.street_Address1,
                'delivery_street_address2': customer_object.street_Address2,
                'delivery_town_or_city': customer_object.town_or_city,
                'delivery_county': customer_object.county,
                'delivery_country': customer_object.country,
                'delivery_postcode': customer_object.postcode,
                'delivery_phone_number': customer_object.phone_number,
